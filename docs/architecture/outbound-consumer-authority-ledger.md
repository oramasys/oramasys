...
The pinned Core revision also requires a serialized `graph_id` during validated
`GraphSpec.from_dict()` reconstruction. That is an integrity boundary for the
injected provider-dispatch graph, not a new outbound authority: Oramasys still
has no concrete provider client and retains no dispatch record. The pin and its
required-identity regression are maintained in
`src/tests/test_perpetua_graph_spec.py`.
